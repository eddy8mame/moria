...
const dataRoot =
    process.env.DATA_DIR || path.resolve(__dirname, '../data/raw/');
const plantPath = path.join(dataRoot, 'eia860', 'Plant_Y2024.xlsx');
const genPath = path.join(dataRoot, 'eia860', 'Generator_Y2024.xlsx');

const DATABASE_URL = `postgresql://${process.env.PG_USER}:${process.env.PG_PASSWORD}@${process.env.DB_HOST}:${process.env.DB_PORT}/${process.env.PG_DB}`;
...
async function ingestEIA() {
    ...
    if (!fs.existsSync(plantPath) || !fs.existsSync(genPath)) {
        throw new Error(
            `Files not found. Ensure Plant_Y2024.xlsx and Generator_Y2024.xlsx exist in ${dataRoot}`
        );
    }
    ...
    for (const { name, statusAssigned } of sheetsToProcess) {
        ...
        if (!sheet) {
            console.warn(
                `Warning: Sheet '${name}' not found in Generator file.`
            );
            continue;
        }
    ...
    try {
        ...
        await client.query(
            'TRUNCATE TABLE us_power_plants RESTART IDENTITY CASCADE'
        );
        ...
        for (const plant of plantsRaw) {
            ...
            const operatingStatus = Object.keys(agg.status_weights).reduce(
                (a, b) =>
                    agg.status_weights[a] > agg.status_weights[b] ? a : b,
                'Unknown'
            );
