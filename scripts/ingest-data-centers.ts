...
async function ingest() {
    ...
    try {
        ...
        const batchSize = 500;
        let batchPromises: Promise<QueryResult>[] = [];
      ...
      `;
            ...
            const executionPromise = client
                ...
                .catch((err) => {
                    ...
                    console.error(
                        `Facility: ${row.facility_name || 'Unknown'}`
                    );
                    console.error(`Error: ${(err as Error).message}`);
                    throw err;
                });

            batchPromises.push(executionPromise);

            // Execute batch when threshold is met
            if (batchPromises.length >= batchSize) {
                await Promise.all(batchPromises);
                inserted += batchPromises.length;
                batchPromises = [];
            }
        }
        ...
        console.log(`Done: ${inserted} data centers inserted successfully.`);
    } catch (err) {
        await client.query('ROLLBACK');
        console.error(
            '\nIngestion failed, entire transaction rolled back.',
            err
        );
        process.exit(1);
    } finally {
